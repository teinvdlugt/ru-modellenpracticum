...
namespace CompuCell3D {
	...
	class /*DECLSPECIFIER*/ Potts3D :public SteerableObject {
		...
		std::vector<unsigned char> eatableTypeVec;///lists types which will remain frozen throughout the simulation
		unsigned int sizeEatableTypeVec;
	...
	public:
		...
		bool checkIfEatable(unsigned char _type);
		...
		void setEatableTypeVector(std::vector<unsigned char> & _eatableTypeVec);
		const std::vector<unsigned char> & getEatableTypeVector() { return eatableTypeVec; }
